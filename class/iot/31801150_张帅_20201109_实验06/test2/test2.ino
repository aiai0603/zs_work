...
#endif
const char* ssid = STASSID; const char* password = STAPSK; ESP8266WebServer server(80);
...
if (cookie.indexOf("ESPSESSIONID=1") != -1) { Serial.println("Authentication Successful"); return true;
}
}
...
if (server.arg("USERNAME") == "admin" &&  server.arg("PASSWORD") == "admin") { server.sendHeader("Location", "/");
server.sendHeader("Cache-Control", "no-cache"); server.sendHeader("Set-Cookie", "ESPSESSIONID=1"); server.send(301);
...
content += "You also can go <a href='/inline'>here</a></body></html>"; server.send(200, "text/html", content);
}

//root page can be accessed only if authentication is ok 
void handleRoot() {
Serial.println("Enter handleRoot"); String header;
if (!is_authenticated()) { server.sendHeader("Location", "/login"); server.sendHeader("Cache-Control", "no-cache"); server.send(301);
return;
}
String  content = "<html><body><H2>hello, you successfully  connected to esp8266! by31801150 zhangshuai</H2><br>";
...
String  content = "<html><body><H2>hello, you successfully  connected to gpio0! by31801150 zhangshuai</H2><br>";
...
String  content = "<html><body><H2>hello, you successfully  connected to gpio1! by31801150 zhangshuai</H2><br>";
